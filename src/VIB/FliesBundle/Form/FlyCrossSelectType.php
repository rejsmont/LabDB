...
/**
 * FlyCrossSelectType class
 *
 * @author Radoslaw Kamil Ejsmont <radoslaw@ejsmont.net>
 */
class FlyCrossSelectType extends AbstractType
{  
    /**
     * Get name
     *
        ...
        $refreshEntities = function ($form, $selections) use ($factory) {
            ...
            $queryBuilder = function (EntityRepository $repository) use ($selections) {
                if (count($selections) < 1)
                    $selections = array(0);
                $qb = $repository->createQueryBuilder('cross')
                        ->where('cross.id in (:values)')
                        ->setParameter('values', $selections);
                return $qb;
            };
            
            $form->add($factory->createNamed('entity','items',null, array(
                'class'         =>  'VIB\FliesBundle\Entity\FlyCross',
                'multiple'      =>  true,
                'expanded'      =>  true,
                'required'      =>  false,
